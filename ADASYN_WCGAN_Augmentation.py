...
# Variabile globale pentru Income_Category – se vor seta în main ca float
INCOME_MIN = None
INCOME_MAX = None
...
def preprocess_data(df):
    ...
    # Eliminăm sufixele 'months' și 'years' și convertim la numeric
...
def enforce_constraints(row):
    ...
    # Grupuri mutual exclusive: Expense Distribution
    ...
    # Grupuri mutual exclusive: Credit
    credit_cols = [
        'Credit_Essential_Needs', 'Credit_Major_Purchases', 'Credit_Unexpected_Expenses',
        'Credit_Personal_Needs', 'Credit_Never_Used'
    ]
    ...
    # Constrângeri pentru Essential_Needs_Percentage
    try:
    ...
    # Income_Category: dacă nu este validă, se atribuie o valoare aleatorie între INCOME_MIN și INCOME_MAX
    try:
    ...
    # Dependențe între Debt_Level și Financial_Investments
    ...
    # Dependență între Impulse_Buying_Frequency și Impulse_Buying_Reason
...
def main():
    ...
    # Setăm valorile minime și maxime pentru Income_Category ca float
    INCOME_MIN = float(df['Income_Category'].min())
    INCOME_MAX = float(df['Income_Category'].max())

    # ADASYN: procesăm datele folosind pipeline-uri pentru categorice și numerice
    ...
    # Antrenăm CTGAN folosind datele combinate (cu label inclus)
